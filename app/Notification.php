...
class Notification extends Model {
     ...
     * @var array
     */
    protected $fillable = ['notification_uuid', 'sender_id', 'receiver_id', 'uuid', 'message', 'name', 'date', 'purchase_time', 'class_schedule_uuid', 'package_uuid', 'notification_type', 'is_read', 'is_archive', 'created_at', 'updated_at'];
    ...
    public function senderFreelancer() {
        return $this->hasOne('App\Freelancer', 'freelancer_uuid', 'sender_uuid')->where('is_archive', '=', 0);
    }

    public function senderCustomer() {
        return $this->hasOne('App\Customer', 'customer_uuid', 'sender_uuid')->where('is_archive', '=', 0);
    }
    ...
    protected function checkNotification($inputs = []) {
        $result = Notification::where('sender_uuid', '=', $inputs['sender_uuid'])
                ->where('receiver_uuid', '=', $inputs['receiver_uuid'])
    ...
    protected function updateNotification($inputs = [], $data) {
        if (Notification::where('sender_uuid', '=', $inputs['sender_uuid'])->where('receiver_uuid', '=', $inputs['receiver_uuid'])->where('notification_type', '=', $inputs['notification_type'])->exists()) {
            $result = Notification::where('sender_uuid', '=', $inputs['sender_uuid'])->where('receiver_uuid', '=', $inputs['receiver_uuid'])->where('notification_type', '=', $inputs['notification_type'])->update($data);
