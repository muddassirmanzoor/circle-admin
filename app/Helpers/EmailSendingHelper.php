...
//        return Mail::to($model)->send(new RecoverEmail($model));
    ...
    public static function sendPayoutFailedEmailToFreelancer($data = []) {
            $data['subject'] = "Circl - Payout Failed";
            self::sendMail('emails.payout_status',$data);
    }

    public static function sendPayoutFailedEmailToAdmin($data = []) {
        $data['subject'] = "Circl - Payout Failed";
        $data['template'] = 'emails.add_multiple_class';
        return self::sendMail('emails.payout_failed',$data);
    }

    public static function sendMail($template, $data, $attachment = null)
    {
        try {
            $support_email  = self::emails('support_email');
            $site_title     = self::emails('site_title');
            $mail = Mail::send($template, ['data' => $data], function ($message) use ($support_email, $site_title, $data, $attachment) {
                $message->from($support_email, $site_title);
                $message->subject($data['subject'] ?? 'Circl');
                $message->to($data['email']);
                if($attachment) {
                    // $message->attach(storage_path('email-attachments/nuaris_agreement.pdf'), array('as' => time() . '.pdf', 'mime'  => 'application/pdf'));
                }
            });
    ...
    {
        $list = [
            'admin_email'   => config('general.emails.ADMIN_EMAIL'),
            'support_email' => config('general.emails.SUPPORT_EMAIL'),
            'from_email'    => config('general.emails.MAIL_FROM_ADDRESS'),
            'site_title'    => config('general.emails.MAIL_FROM_NAME'),
        ];
