...
Class ClassHelper {
    ...
    public static function getClassDetails($inputs = []) {
        ...
        $response = ClassResponseHelper::classDetailsResponse($class_detail, $inputs['local_timezone'], $inputs['logged_in_uuid']);
    ...
    public static function getSingleDayClass($inputs = []) {
        ...
        $response = ClassResponseHelper::singleDayClassResponse($class_detail, $inputs['date'], $inputs);
    ...
    public static function freelancerAddClassProcess($inputs, $class, $inputs_array) {
        foreach ($inputs_array as $key => $day_schedule) {
            ...
            unset($inputs_array[$key]['freelancer_uuid']);
...
//    }
    ...
    public static function getActiveClassesCount($inputs = []) {
        ...
        $total_class_count = 0;
        $class_list = Classes::getFreelancerUpcomingClasses('freelancer_uuid', $inputs['freelancer_uuid'], date('Y-m-d'));
        if (!empty($class_list)) {
    ...
    public static function getClassesList($inputs = []) {
        ...
        $class_list_response = Classes::getFreelancerUpcomingClasses('freelancer_uuid', $inputs['freelancer_uuid'], date('Y-m-d'));
...
//        }
    ...
    public static function changeClassStatus($inputs = []) {
        ...
        if ($inputs['login_user_type'] == "customer") {
            ...
            $updateClassStatus = ClassBooking::updateClassBooking($inputs['class_schedule_uuid'], $inputs['logged_in_uuid'], ["status" => "cancelled"]);
            ...
            } elseif ($inputs['logged_in_uuid'] != $class_detail['freelancer_uuid']) {
            ...
            $updateClassBooking = ClassBooking::updateClassBookings('class_schedule_uuid', $inputs['class_schedule_uuid'], ["status" => "cancelled"]);
            if (!$updateClassBooking) {
                DB::rollBack();
                return CommonHelper::jsonErrorResponse(ClassValidationHelper::changeClassStatusRules()['message_' . strtolower($inputs['lang'])]['update_class_error']);
            }

            $updateClassSchedule = ClassSchedule::updateClassSchedule('class_schedule_uuid', $inputs['class_schedule_uuid'], ['status' => 'cancelled']);
        ...
        if ($inputs['login_user_type'] == "customer" && $inputs['status'] == "cancelled" && $updateClassStatus) {
            $check = self::sendEmailtoFreelancerforStatusUpdate($inputs['class_uuid'], $inputs['class_schedule_uuid'], $inputs['logged_in_uuid'], $inputs['status']);
        }
...
//    }
    ...
    public static function getAvailableClasses($inputs) {
        ...
        $date = date('Y-m-d');
        $class_list_response = Classes::getFreelancerUpcomingClasses('freelancer_uuid', $inputs['freelancer_uuid'], $date);
        if (empty($class_list_response)) {
            return CommonHelper::jsonSuccessResponse(MessageHelper::getMessageData('error', $inputs['lang'])['invalid_data']);
        }
    ...
    public static function searchClasses($inputs = []) {
        ...
        if (!empty($inputs['freelancer_uuid'])) {
            $class_response = Classes::searchClasses('freelancer_uuid', $inputs['freelancer_uuid'], $inputs['search_params'], (!empty($inputs['limit']) ? $inputs['limit'] : null), (!empty($inputs['offset']) ? $inputs['offset'] : null));
        }
    ...
    public static function getFreelancerUpcomingClassSchedules($inputs = [], $data_filters = []) {
        $date = date('Y-m-d');
        $upcoming_classes = Classes::getFreelancerUpcomingClasses('freelancer_uuid', $inputs['profile_uuid'], $date);
    ...
    public static function getCustomerUpcomingClassBookings($inputs = [], $data_filters = []) {
        ...
        $pending_booked_classes = ClassBooking::pluckClassBookingIds('customer_uuid', $inputs['profile_uuid'], 'class_schedule_uuid', 'pending');
        ...
        $confirmed_booked_classes = ClassBooking::pluckClassBookingIds('customer_uuid', $inputs['profile_uuid'], 'class_schedule_uuid', 'confirmed');
