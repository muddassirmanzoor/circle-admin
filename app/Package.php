...
class Package extends Model
{
    ...
    protected $fillable = ['package_uuid', 'freelancer_id', 'package_name', 'package_type', 'no_of_session', 'discount_type', 'discount_amount', 'currency', 'price', 'discounted_price', 'package_image', 'package_validity', 'validity_type', 'package_description', 'description_video', 'description_video_thumbnail', 'is_archive'];
    ...
    protected function getAllPackages($column, $value, $limit = null, $offset = null)
    {
        ...
        $query = $query->withCount('PackageClass');
    ...
    protected function getSinglePackage($column, $value)
    {
        $query = Package::where($column, '=', $value)->where('is_archive', 0);
        $query = $query->with('PackageService.FreelancerCategory.SubCategory');
        $query = $query->with('PackageService.FreelancerCategory.Category');
        $query = $query->with('PackageAppointment');
        $query = $query->with('ClassBooking.classObject');
        $query = $query->with('ClassBooking.customer');
        $query = $query->with('PackageClass.Classes.freelancer');
        $query = $query->with('PackageClass.Classes.singleSchedule');
        $query = $query->with('PackageClass.Classes.FreelanceCategory.SubCategory');
        $query = $query->with('Freelancer');
    ...
    //        $query = $query->with('PackageAppointment.AppointmentCustomer', 'PackageAppointment.AppointmentFreelancer', 'PackageAppointment.AppointmentServices', 'PackageAppointment.AppointmentFreelancer');
    ...
    {
        $query = Package::whereIn('package_uuid', $uuid_array)->where('is_archive', 0);
        $query = $query->with('Freelancer.profession');
        $query = $query->with('PackageAppointment.AppointmentCustomer', 'PackageAppointment.AppointmentFreelancer', 'PackageAppointment.AppointmentServices', 'PackageAppointment.AppointmentFreelancer');
        $query = $query->with('ClassBooking.customer', 'ClassBooking.classObject', 'ClassBooking.schedule.classBookings.customer');
        $query = $query->orderBy('id', 'desc');
    ...
    protected function getPurchasedPackageDetails($column, $value, $query_data = [])
    {
        ...
        //            }
        //        });
        $query = $query->with('Freelancer', 'PackageAppointment.AppointmentCustomer', 'PackageAppointment.AppointmentFreelancer', 'PackageAppointment.AppointmentServices', 'PackageAppointment.AppointmentFreelancer', 'ClassBooking.customer', 'ClassBooking.classObject', 'ClassBooking.schedule.classBookings.customer');
        $result = $query->first();
        return !empty($result) ? $result->toArray() : [];
    }
    ...
    public static function getAllSessionPackages()
    {
        ...
        $query = $query->with('PackageService.FreelancerCategory.SubCategory');
        $query = $query->with('PackageAppointment.AppointmentCustomer', 'PackageAppointment.AppointmentFreelancer.profession', 'PackageAppointment.AppointmentServices');
        $query = $query->with('Freelancer');
        $query = $query->orderBy('id', 'desc');
        if (!empty($offset)) {
            $query = $query->offset($offset);
