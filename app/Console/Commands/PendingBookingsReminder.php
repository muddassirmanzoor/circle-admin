...
class PendingBookingsReminder extends BaseCommand {
     ...
     * @var string
     */
    protected $commandName = 'Pending Booking Reminder';
    /**
     ...
     * @var string
     */
    protected $logChannel = 'cron_pending_booking_reminder';
    ...
    public function handle() {
        ...
        if (!$send_notifications['success']) {
            $this->log('Process failed with exception', [
                'message' => $send_notifications['message']
            ], 'error');
            $this->InsertDBLog(0);
        }
        else{
            $this->log($send_notifications['message']);
//            DB::rollBack();
            $this->InsertDBLog();
        }
    ...
    public function automatePendingAppointmentProcess($pending_appointments) {
        ...
        foreach ($pending_appointments as $appointment) {
            if ($appointment['status'] == 'pending') {
                $this->log('Appointment date check:', [
...
//        $time_after_update = date('H:i:s', $updated_time);
        ...
        $this->log('Appointment 6 hours & 1 hour time check:', [
            'appointment_time' => $appointment['from_time'],
            'current_time' => date('H:i:s'),
            'six_hours_condition' => (strtotime($appointment['from_time']) >= strtotime('-10 minutes', strtotime($time_after_six_hour)) && strtotime($appointment['from_time']) < strtotime('+10 minutes', strtotime($time_after_six_hour)) ),
            'one_hour_condition' => ( strtotime($appointment['from_time']) >= strtotime('-10 minutes', strtotime($time_after_one_hour)) && strtotime($appointment['from_time']) < strtotime('+10 minutes', strtotime($time_after_one_hour)) ),
        ]);
        if ((strtotime($appointment['from_time']) >= strtotime('-10 minutes', strtotime($time_after_six_hour)) && strtotime($appointment['from_time']) < strtotime('+10 minutes', strtotime($time_after_six_hour))) ||
            ( strtotime($appointment['from_time']) >= strtotime('-10 minutes', strtotime($time_after_one_hour)) && strtotime($appointment['from_time']) < strtotime('+10 minutes', strtotime($time_after_one_hour)) )) {
            $this->log('Sending Pending Appointment Push Notification:', [
